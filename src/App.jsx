...
function ListingCard({ listing, setPage }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ height: '72px', background: listing.hasDeal ? '#1a0a0a' : '#1a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', position: 'relative' }}>
        🍕
...
function Nav({ page, setPage }) {
  ...
  const link = (label, target) => (
    <span
      onClick={() => { setPage(target); setMobileOpen(false) }}
  ...
  return (
    <div>
      ...
      <div style={{ background: G.hero, padding: '52px 24px 44px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.45)', margin: '0 0 28px', fontFamily: G.font }}>30+ spots. Real ratings. Order in one click.</p>
        ...
        <div style={{ display: 'flex', gap: '36px', justifyContent: 'center', marginTop: '22px' }}>
          {[['30+', 'Pizza Spots'], [LISTINGS.filter(l=>l.deal).length, 'Live Deals'], [NEIGHBORHOODS.length, 'Neighborhoods']].map(([n, l]) => (
      ...
      <div style={{ padding: '32px 24px 48px', background: G.bgDeep, maxWidth: '720px', margin: '0 auto' }}>
        ...
        {[
          ...
          ['How do I find pizza deals near me in Brooklyn?', 'Check our Deals page for this week\'s active promos across 30+ Brooklyn spots. We update it weekly with DoorDash, Grubhub, and Slice offers.'],
          ...
          ['What neighborhoods have the best pizza in Brooklyn?', 'Williamsburg, Carroll Gardens, DUMBO, Midwood, and Bensonhurst have the highest concentration of great pizza spots.'],
        ].map(([q, a]) => (
